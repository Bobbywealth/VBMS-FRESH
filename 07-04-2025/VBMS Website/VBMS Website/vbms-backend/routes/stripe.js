...
router.post('/create-customer', authenticateToken, async (req, res) => {
  try {
    ...
    if (user.billing.stripeCustomerId) {
      return res.json({ customerId: user.billing.stripeCustomerId });
    }
...
router.post('/create-subscription', authenticateToken, async (req, res) => {
  try {
    ...
    const vbmsSubscription = new Subscription({
      customerId: user._id,
      packageType: packageType,
    ...
    await vbmsSubscription.save();
    
    // Update user
    user.subscription = vbmsSubscription._id;
...
router.get('/subscription', authenticateToken, async (req, res) => {
  try {
    ...
    const stripeSubscription = await stripe.subscriptions.retrieve(
      user.subscription.billing.stripeSubscriptionId
    );
...
router.post('/update-subscription', authenticateToken, async (req, res) => {
  try {
    ...
    user.subscription.packageType = newPackageType;
    user.subscription.packageName = newPlan.name;
    user.subscription.price.monthly = newPlan.price;
    user.subscription.price.perCall = newPlan.perCall || 0;
